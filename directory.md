...
`src` 目录用于存放源码文件，必须根据`业务逻辑`划分的目录结构，我们称为`业务目录`。
...
- 静态资源
  - `入口资源` 前端入口页面、后端模板页面
  - `模块化资源` js模块、css模块或组件
    - `源代码资源`：指开发者编写的源代码，包括`js`、`css`、`tpl`等。
    - `内容资源`：指希望做为内容提供给访问者的资源，包括`img`、`font`、`swf`、`pdf`等。
  - `非模块化资源` 并不是所有的开发资源都是模块化的，比如提供模块化框架的js本身就不能是一个模块化的js文件。
    - `源代码资源`：指开发者编写的源代码，包括`js`、`css`、`tpl`等。
    - `内容资源`：指希望做为内容提供给访问者的资源，包括`img`、`font`、`swf`、`pdf`等。
- 动态资源
...
`dist` 目录用于存放部署文件，应当根据`业务职能`划分目录结构。


1. `JS资源` 不得按`文件类型`划分目录， *必须(MUST)* 按`业务逻辑`划分目录。`JS资源`应直接置于`业务目录`下。即：`业务目录`下禁止出现`js`目录。
...
src/按业务逻辑划分/业务职能/业务场景/业务分层/业务分块/按文件类型划分
...
`src`目录下 *必须(MUST)* 只包含`业务目录`与`common`目录。`业务公共资源` *必须(MUST)* 命名为`common`。`common`目录作为`业务公共资源`的目录，也视如`业务目录`。
