...
　[5.1 按项目职能划分](#51-按项目职能划分)

　[5.2 按开发概念划分](#52-按开发概念划分)

　[5.3 按业务逻辑划分](#53-按业务逻辑划分)

　[5.4 按文件类型划分](#54-按文件类型划分)

[6 目录划分说明](#6-目录划分说明)

  [6.1 一级目录划分](#61-一级目录划分)
  
  [6.2 src 目录划分](#62-src-目录划分)
  
  [6.3 dist 目录划分](#63-dist-目录划分)

[7 src 和 dist 目录参考示例](#7-src-和-dist-目录参考示例)
...
根据资源的运行环境、语言特性、业务场景等设计语义化开发概念，定义开发资源的分类。

### 5.3 按业务逻辑划分

#### 使用场景

用于模块的目录划分。根据`业务逻辑`划分的目录，我们称为`业务目录`。
...
- 业务模块文件数量过多时，并且无法划分子业务目录时候，请参考[5.3 按业务逻辑划分](#53-按业务逻辑划分)。
- 部署目录中静态资源的划分。
...
设计开发概念，定义开发资源的分类，[按开发概念划分](#52-按开发概念划分) `src` 目录。
