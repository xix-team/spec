...
2. 不得使用复数形式，目录本身就用于存放多个文件，不需要使用多余的复数形式。如：`imgs`、`docs`、`images`、`tpls`、`examples`、`tests`是不被允许的。
...
直接置于`$root`下的目录称作`一级目录`。`一级目录` *必须(MUST)* 按照`职能`划分， 不得将`资源类型`或`业务逻辑`划分的目录直接置于`$root`下。
